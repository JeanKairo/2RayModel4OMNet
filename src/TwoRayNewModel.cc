...
TwoRayNewModel::TwoRayNewModel() :
    epsilon_r(1.0), polarization('v'), sigma(1.0), delta_h(1.0)
{
}
...
std::ostream& TwoRayNewModel::printToStream(std::ostream& os, int level, int evFlags) const
{
    os << "TwoRayNewModel";
...
double TwoRayNewModel::terraEsferica(const Coord& pos_t, const Coord& pos_r, m lambda) const
{
    ...
    const std::complex<double> sqrt_term = sqrt(42.0 - sen_theta_complex*sen_theta_complex);
    switch (polarization)
    {
        case 'h':
        ...
        case 'v':
            ...
            Gamma = (-42.0 * cos_theta_complex + sqrt_term) / (42.0* cos_theta_complex + sqrt_term);
            break;
        default:
